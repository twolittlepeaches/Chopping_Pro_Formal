%%  DTC Chopping / Differential DTC / Dither PLL 行为级仿真  v5
%
%  v5 修改说明（相对 v4）：
%    1. 删除 LMS2_EN 模块（LUT-LMS）
%    2. 新增 NCC_EN 开关：
%         NCC_EN = 0  → 保持 v4 原有 sign-LMS 校准
%         NCC_EN = 1  → 三路校准切换为 NCC 二分搜索
%                        顺序：热身 → KOFT → KDTC → POLY → 慢速跟踪
%                        约 4000 周期收敛，与 FCW_F 无关
%    3. Plot 7 校准收敛图保持原有风格，标题注明当前模式
% ================================================================
...
NCC_EN        = 0;   % 【新增】0=sign-LMS（v4原版），1=NCC二分搜索
...
if NCC_EN
    fprintf('NCC模式：总收敛 %d 周期 = %.1f μs\n', T_end_poly, T_end_poly/fref*1e6);
end
...
for step = 1:Sim_Time
    ...
    if ~NCC_EN
        ...
        %  相关信号与更新方向（推导见注释）：
        %
        %  [KOFT] corr = sign(SWAP-0.5) × sign(PHE)
        %    Koft 偏大时：SWAP=0 → PHE 正偏，SWAP=1 → PHE 负偏
        %    → corr 均值 < 0 → Koft 需减小 → 更新：Koft += sign(acc) × step
        %
        %  [KDTC] corr = sign(PHE) × sign(PHRF)
        %    增益偏大时：PHE 与 PHRF 同号（过补偿）→ acc > 0
        %    更新：gain -= sign(acc) × step
        %
        %  [POLY] corr = sign(PHE) × sign(poly_val)
        %    alpha 偏小时：残余奇 INL 与 poly_val 同向 → acc > 0
        %    更新：alpha_odd += sign(acc) × step

        if step <= T_end_warmup
            %% Phase 0: 热身，冻结所有校准，等 PLL 粗锁

        elseif step <= T_end_koft
            %% Phase 1: NCC KOFT
            if CHOPPING_EN
                ncc_acc = ncc_acc + sign(EN_SWAP(step) - 0.5) * sign(phie(step));
                ncc_cnt = ncc_cnt + 1;
                if ncc_cnt >= NCC_M
                    Koft          = Koft + sign(ncc_acc) * ncc_step_koft;
                    ncc_step_koft = ncc_step_koft / 2;
                    ncc_acc = 0;  ncc_cnt = 0;
                end
            end
            if step == T_end_koft; ncc_acc = 0; ncc_cnt = 0; end

        elseif step <= T_end_kdtc
            %% Phase 2: NCC KDTC
            ncc_acc = ncc_acc + sign(phie(step)) * sign(PHRF_cur + eps);
            ncc_cnt = ncc_cnt + 1;
            if ncc_cnt >= NCC_M
                gain_DTC_est  = gain_DTC_est - sign(ncc_acc) * ncc_step_kdtc;
                ncc_step_kdtc = ncc_step_kdtc / 2;
                ncc_acc = 0;  ncc_cnt = 0;
            end
            if step == T_end_kdtc; ncc_acc = 0; ncc_cnt = 0; end

        elseif step <= T_end_poly && LMS_POLY_EN
            %% Phase 3: NCC alpha_odd
            if poly_val ~= 0
                ncc_acc = ncc_acc + sign(phie(step)) * sign(poly_val);
                ncc_cnt = ncc_cnt + 1;
            end
            if ncc_cnt >= NCC_M
                alpha_odd     = alpha_odd + sign(ncc_acc) * ncc_step_poly;
                ncc_step_poly = ncc_step_poly / 2;
                ncc_acc = 0;  ncc_cnt = 0;
            end
            if step == T_end_poly; ncc_acc = 0; ncc_cnt = 0; end

        else
            %% Phase 4: 慢速跟踪（步长 << NCC 末位精度，追踪温漂）
            if CHOPPING_EN
                Koft = Koft + MU_TRACK_KOFT * sign(EN_SWAP(step) - 0.5) * sign(phie(step));
            end
            gain_DTC_est = gain_DTC_est - MU_TRACK_KDTC * sign(phie(step)) * sign(PHRF_cur + eps);
            if LMS_POLY_EN && poly_val ~= 0
                alpha_odd = alpha_odd + MU_TRACK_POLY * sign(phie(step)) * poly_val;
            end
        end
...
% ============================================================
if NCC_EN
    settle_start_fref = T_end_poly + 500;
else
...
% ============================================================
%%  Plot 7: 校准收敛曲线（与 v4 相同风格，标题注明模式）
% ============================================================
if 1
    ...
    if NCC_EN
        mode_str = sprintf('NCC二分 (%d bits × %d M)', NCC_BITS_KOFT, NCC_M);
    else
        mode_str = 'sign-LMS';
    end
...
if NCC_EN
    fprintf('\nNCC 收敛结果:\n');
    fprintf('  总时间: %d 周期 = %.1f μs\n', T_end_poly, T_end_poly/fref*1e6);
    fprintf('  Koft      = %6.2f ps  (初始 300ps，末位步长 %.2f fs)\n', ...
        Koft*1e12, ncc_step_koft*1e15);
    fprintf('  gain_DTC  = %8.1f   (理论 %8.1f)\n', gain_DTC_est, Tv/DTC_reso);
    if LMS_POLY_EN
        fprintf('  alpha_odd = %6.3f fs  (末位步长 %.3f fs)\n', ...
            alpha_odd*1e15, ncc_step_poly*1e15);
    end
end
